      ...
      <div style={{  color: "#4D340A" }} className="container px-5 w-100">
        ...
        <div className="row justify-content-center">
          <div className="col-lg-8 col-md-10 col-sm-12">
            ...
            <input value={movieDetails.mTitle} onChange={e=>setMovieDetails({...movieDetails, mTitle: e.target.value})} type="text" className="form-control mb-5" />
  
            <h5 >Category: {invalidCategory &&<span className="text-danger">*</span>} </h5>
            <input value={movieDetails.mCategory} onChange={(e) => {const newCategory = e.target.value;setMovieDetails({ ...movieDetails, mCategory: newCategory });categoryValidate();}} type="text" className="form-control mb-5" />
  
            <h5>Director's Name: {invalidDirName &&<span className="text-danger">*</span>}</h5>
            <input value={movieDetails.dirName} onChange={(e) => {const newDirName = e.target.value;setMovieDetails({ ...movieDetails, dirName: newDirName });dirNameValidate();}} type="text" className="form-control mb-5" />
  
            <h5>Poster Link: {invalidImgUrl && <span className="text-danger">*</span>}</h5>
            <input value={movieDetails.imgUrl}  onChange={(e) => {const newImgUrl = e.target.value; setMovieDetails({ ...movieDetails, imgUrl: newImgUrl });validateImage(newImgUrl);}} type="text" className="form-control mb-5" placeholder="https://..." />
  
            <h5>Movie Trailer Link: {invalidVideo && <span className="text-danger">*</span>} </h5>
            <input value={movieDetails.uTubeLink} onChange={(e) => {const newVideo = e.target.value; setMovieDetails({ ...movieDetails, uTubeLink: newVideo });validateVideo(newVideo);}} type="text" className="form-control mb-5" placeholder="https://..." />
  
            <h5>Detailed Review: </h5>
            <textarea value={movieDetails.mReview} onChange={e=>setMovieDetails({...movieDetails, mReview: e.target.value})} className="form-control mb-5" style={{ height: "200px" }}></textarea>
           ...
           <div className='d-flex justify-content-between'>
            <button onClick={handlemovieDetails} disabled={invalidForm} className="btn mb-5"   style={{backgroundColor: "#E5D0AC" ,color: "#4D340A" ,height: "40px",width: "150px"}}>Upload Review</button>
            
            <button onClick={handleReset} className="btn mb-5"  style={{backgroundColor: "#E5D0AC" ,color: "#4D340A" ,height: "40px",width: "100px"}}>Reset</button>
            </div>
          </div>
        </div>
